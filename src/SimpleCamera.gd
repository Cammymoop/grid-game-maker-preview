	...
	if follow_this:
		if by_mode == "name":
			...
			if target_entity:
				...
				push_warning("No entity found for camera follow by name: " + follow_this + " " + str(ent_index))
		elif by_mode == "property":
			target_entity = EntityManager.find_entity_with_property(follow_this, false)
			if target_entity:
				ent_center_offset = target_entity.get_center_offset()
			else:
				push_warning("No entity found for camera follow by property: " + follow_this)
		else:
