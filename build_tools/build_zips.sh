...
EXAMPLE_GAMES_DIR="${PROJECT_DIR}/example_games"
GODOT_BIN="godot4.6"
...
if [[ ! -d "${EXAMPLE_GAMES_DIR}" ]]; then
  echo "Error: custom levels dir not found at ${EXAMPLE_GAMES_DIR}" >&2
  exit 1
fi

echo "EXPORTING IN ${EXP_MODE} MODE"
...
while IFS='|' read -r PRESET_NAME OUT_SUBDIR OUT_FILE <&3; do
  ...
  "${GODOT_BIN}" --headless --path "${PROJECT_DIR}" \
    --export-"${EXP_MODE}" "${PRESET_NAME}" "${DEST_PATH}" | tail -n 15 | cat <(echo "...") -
...
for OUT_SUBDIR in "${EXPORTED_SUBDIRS[@]}"; do
  ...
  (
    cd "$(dirname "${EXAMPLE_GAMES_DIR}")"
    zip -qr "${ZIP_PATH}" "$(basename "${EXAMPLE_GAMES_DIR}")"
  )
done
